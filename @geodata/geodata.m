classdef geodata
    ...
    properties
        ...
        linestrings % unclosed vector features (e.g., thalwegs)
    ...
    methods
        ...
        function obj = ParseShoreline(obj)
            ...
            if ~isempty(obj.contourfile)
                ...
                obj.linestrings = polygon_struct.linestrings;
        ...
        function plot(obj, type, projection, holdon)
            ...
            %
            % Optional inputs:
            % type : 'shp' [default] - plots the shapelines (shoreline) only,
            %        'dem' - plots the DEM bathy in addition to shapelines,
            %        'omega' - hatches the meshing domain in addition to plotting shapelines.
            % projection : choose the projection type from m_map options (default is Mercator)
            ...
            if ~holdon
                ...
                if ~isempty(regexp(projection, 'ste'))
                    m_proj(projection, 'lat', min(obj.bbox(2,:)), 'long', mean(obj.bbox(1,:)), 'radius', ...
                    ...
                    m_proj(projection, 'long', [lon1, lon2], 'lat', [lat1, lat2]);
                end
                figure;
                colori = 'g-';  % island color
                colorm = 'r-';  % mainland color
            else
                colori = 'b-';  % island color for holdon
                colorm = 'm-';  % mainland color for holdon
            end
            hold on

            % Select additional plotting options
            switch type
                case 'dem'
                ...
                case 'omega'
                    ...
                    m_hatch(obj.boubox(1:end-1,1), obj.boubox(1:end-1,2), 'cross', 45, 0.05);
                    m_plot(long, lati, '.', 'Color', 'white');
            end

            % Plot mainland, inner boundaries, and (if available) line strings
            h1 = []; h2 = []; h3 = []; h4 = [];
            ...
            [la, lo] = my_interpm(obj.boubox(:,2), obj.boubox(:,1), 0.5*obj.h0/111e3);
