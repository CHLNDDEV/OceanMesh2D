...
if sum(Crnew > max_cr) > 0
    error('Unable to bound maximum Courant number. Got %4.2f, expecting <%4.2f.',...
        max(Crnew),max_cr);
else
    fprintf('Bounded maximum Courant number. Got %4.2f, expecting <%4.2f.\n',...
        max(Crnew),max_cr);
end
if sum(Crnew < min_cr) > 0
    error('Unable to bound minimum Courant number. Got %4.2f, expecting >%4.2f.',...
        min(Crnew),min_cr);
else
    fprintf('Bounded minimum Courant number. Got %4.2f, expecting >%4.2f.\n',...
        min(Crnew),min_cr);
end
